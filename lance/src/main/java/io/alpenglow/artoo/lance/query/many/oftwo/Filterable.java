...
public interface Filterable<A, B> extends Queryable.OfTwo<A, B> {
  default Many.OfTwo<A, B> where(final TryBiPredicate<? super A, ? super B> where) {
    return where((index, first, second) -> where.tryTest(first, second));
  }

  default Many.OfTwo<A, B> where(final TryTriPredicate<? super Integer, ? super A, ? super B> where) {
