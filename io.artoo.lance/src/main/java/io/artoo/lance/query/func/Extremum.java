...
public final class Extremum<T, N extends Number, V> implements Func.MaybeFunction<T, V> {
  private final class Extremed { private V value; }

  private final Extremed extremed;
  private final int extreme;
  private final Func.MaybeFunction<? super T, ? extends N> number;

  public Extremum(final int extreme, final Func.MaybeFunction<? super T, ? extends N> number) {
    assert number != null;
