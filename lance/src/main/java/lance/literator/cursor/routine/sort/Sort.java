...
public sealed interface Sort<T> extends Routine<T, Cursor<T>> permits Default, Arranged {
  record By<T, R>(Func.TryFunction<? super T, ? extends R> field, Ordering.Arrange arrange) {
    public By(Func.TryFunction<? super T, ? extends R> field) {
      this(field, asc);
    }

    public static <T, R> By<T, R> with(Func.TryFunction<? super T, ? extends R> field, Ordering.Arrange arrange) {
