...
public interface Peekable<A, B> extends Queryable.OfTwo<A, B> {
  default Many.OfTwo<A, B> peek(Cons.TryBiConsumer<? super A, ? super B> peek) {
    return peek((index, first, second) -> peek.tryAccept(first, second));
  }

  default Many.OfTwo<A, B> peek(Cons.TryTriConsumer<? super Integer, ? super A, ? super B> peek) {
    return () -> cursor().map(new Peek<Pair<A, B>, Pair<A, B>>((index, record) -> peek.tryAccept(index, record.first(), record.second())));
  }

  default Many.OfTwo<A, B> exceptionally(Cons.TryConsumer<? super Throwable> catch$) {
