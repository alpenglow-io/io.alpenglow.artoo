...
public interface Numeral<R extends Record & Numeral<R>> {
  ...
  static <R extends Record & Numeral<R>> R from(Number number) {
    ...
    if (number instanceof Float f) return (R) new Decimal32(f);

    if (number instanceof Double d) return (R) new Decimal64(d);
  ...
  static <T extends Record, R extends Record & Numeral<R>> R from$(T record) {
    ...
    if (record instanceof Decimal32 f) return (R) f;

    if (record instanceof Decimal64 d) return (R) d;
  ...
  static <V, R extends Record & Numeral<R>> Function<? super V, ? extends R> asNumeral() {
    return it -> it == null ? null : (R) switch (it.getClass().getSimpleName()) {
      ...
      case "Float" -> new Decimal32((Float) it);
      case "Double" -> new Decimal64((Double) it);
      default -> null;
    };
  }

  @Contract(pure = true)
  static <V> @NotNull Function<? super V, ? extends Decimal64> asSingle64() {
    return it -> it == null ? null : switch (it.getClass().getSimpleName()) {
      case "Byte", "Short", "Integer", "Long", "Float", "Double" -> new Decimal64((Double) it);
      default -> null;
    };
  }
}
