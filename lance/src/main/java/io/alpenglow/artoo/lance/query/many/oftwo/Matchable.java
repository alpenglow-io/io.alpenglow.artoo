...
public interface Matchable<A, B> extends Queryable.OfTwo<A, B> {
  default Many.OfTwo<A, B> when(final TryBiPredicate<? super A, ? super B> pred, TryBiConsumer<? super A, ? super B> cons) {
    return () -> cursor().map(new WhenWhere<>(pair -> pred.tryTest(pair.first(), pair.second()), pair -> cons.tryAccept(pair.first(), pair.second())));
  }

  default <R> Many.OfTwo<A, B> when(final Class<R> type, TryConsumer<? super R> cons) {
    return when(type, unary(cons));
  }

  default <P> Many.OfTwo<A, B> when(final Class<P> type, TryFunction<? super P, ? extends Pair<A, B>> func) {
