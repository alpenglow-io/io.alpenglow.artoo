...
public interface Mappable<T> extends Literator<T> {
  default <R> Cursor<R> map(final Func.TryFunction<? super T, ? extends R> map) {
    return new Map<>(this, map);
  }

  default <R, C extends Cursor<R>> Cursor<R> flatMap(final Func.TryFunction<? super T, ? extends C> flatMap) {
...
final class Map<T, R> implements Cursor<R> {
  ...
  private final Func.TryFunction<? super T, ? extends R> map;

  Map(final Literator<T> source, final Func.TryFunction<? super T, ? extends R> map) {
    this.source = source;
    this.map = map;
  }
  ...
  @Override
  public <P> Cursor<P> map(final Func.TryFunction<? super R, ? extends P> mapAgain) {
