...
public interface Triple<A, B, C> extends Tuple {
  ...
  default <T extends Record> T to(final Func.TryTriFunction<? super A, ? super B, ? super C, ? extends T> to) {
    return to.apply(first(), second(), third());
  }

  default <T> T as(final Func.TryTriFunction<? super A, ? super B, ? super C, ? extends T> as) {
  ...
  default <R extends Record & Triple<A, B, C>> R map(Func.TryTriFunction<? super A, ? super B, ? super C, ? extends R> map) {
    return map.apply(first(), second(), third());
  }

  default <R extends Record & Triple<A, B, C>, F extends Record & Single<R>> R flatMap(Func.TryTriFunction<? super A, ? super B, ? super C, ? extends F> func) {
    return func.apply(first(), second(), third()).first();
  }

  default Triple<A, B, C> peek(Cons.TryTriConsumer<? super A, ? super B, ? super C> cons) {
