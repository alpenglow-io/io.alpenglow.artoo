...
public interface Countable<T> extends Queryable<T> {
  default One<Integer> count() {
    return () -> cursor().foldLeft(0, (counted, element) -> element != null ? counted + 1 : counted);
  }

  default One<Integer> count(TryIntPredicate1<? super T> where) {
    return () -> cursor().filter(where).foldLeft(0, (counted, element) -> element != null ? counted + 1 : counted);
  }
