...
public class QuantifiableTest {
  ...
  public void shouldHaveAnyElement() throws Throwable {
    final var some = Many.from(1, 2).some().cursor().fetch().element();
  ...
  public void shouldNotHaveAnyElement() throws Throwable {
    final var some = Many.from().some().cursor().fetch().element();
  ...
  public void shouldHaveEvenNumber() throws Throwable {
    final var some = Many.from(1, 2).some(number -> number % 2 == 0).cursor().fetch().element();
  ...
  public void shouldHaveThreePeople() throws Throwable {
    final var some = Many.from(owners()).coalesce().some(it -> it.pets().length > 0).cursor().fetch().element();
  ...
  void shouldHaveNone() throws Throwable {
    final var none = Many.from().none().cursor().fetch().element();
  ...
  void shouldHaveNoneIntegerType() throws Throwable {
    final var none = Many.from(true, null, 12.0F, 13.5D, 15L).none(Integer.class).cursor().fetch().element();
  ...
  void shouldHaveNonePets() throws Throwable {
    final var none = Many.from(owners()).coalesce().none(owner -> owner.pets().length > 5).cursor().fetch().element();
