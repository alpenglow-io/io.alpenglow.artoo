...
class ExceptionableTest implements Throwing {
  ...
  void shouldRethrowAnException() {
    assertThrows(IllegalArgumentException.class, () -> {
      for (var element : One.of("element").raise(IllegalArgumentException::new).when(it -> !it.isEmpty())) {
        System.out.println(element);
      }
    });
  }
  ...
  void shouldRecoverAfterAnException() {
    assertDoesNotThrow(() -> {
      for (
        var element
        : One.of("element").select(it -> it.length() > 0 ? throwing(() -> new IllegalStateException("illegal state")) : it).recover(Throwable::getMessage)
      ) {
