...
import io.artoo.parry.InternalThreadLocalMap;
...
import static io.artoo.parry.util.MathUtil.isOutOfBounds;
...
public final class AsciiString implements CharSequence, Comparable<CharSequence> {
  ...
  public AsciiString(ByteBuffer value, int start, int length, boolean copy) {
    ...
    if (value.hasArray()) {
      if (copy) {
        final var bufferOffset = value.arrayOffset() + start;
  ...
  public AsciiString(char[] value, Charset charset, int start, int length) {
    var cbuf = CharBuffer.wrap(value, start, length);
    var encoder = CharsetUtil.encoder(charset);
    var nativeBuffer = ByteBuffer.allocate((int) (encoder.maxBytesPerChar() * length));
    encoder.encode(cbuf, nativeBuffer, true);
    final var bufferOffset = nativeBuffer.arrayOffset();
  ...
  public AsciiString(CharSequence value, Charset charset, int start, int length) {
    var cbuf = CharBuffer.wrap(value, start, start + length);
    var encoder = CharsetUtil.encoder(charset);
    var nativeBuffer = ByteBuffer.allocate((int) (encoder.maxBytesPerChar() * length));
    encoder.encode(cbuf, nativeBuffer, true);
    final var offset = nativeBuffer.arrayOffset();
  ...
  public AsciiString concat(CharSequence string) {
    var thisLen = length();
    var thatLen = string.length();
    if (thatLen == 0) {
      return this;
    }

    if (string instanceof AsciiString) {
      var that = (AsciiString) string;
      if (isEmpty()) {
        return that;
      }
  ...
  public void copy(int srcIdx, char[] dst, int dstIdx, int length) {
    ...
    final var dstEnd = dstIdx + length;
  ...
  public int indexOf(CharSequence subString, int start) {
    final var subCount = subString.length();
    if (start < 0) {
      start = 0;
    }
    if (subCount <= 0) {
    ...
    final var firstChar = subString.charAt(0);
    if (firstChar > MAX_CHAR_VALUE) {
      return INDEX_NOT_FOUND;
    }
    final var firstCharAsByte = c2b0(firstChar);
    final var len = offset + length - subCount;
    for (var i = start + offset; i <= len; ++i) {
  ...
  public int indexOf(char ch, int start) {
    ...
    final var chAsByte = c2b0(ch);
    final var len = offset + length;
    for (var i = start + offset; i < len; ++i) {
      if (value[i] == chAsByte) {
        return i - offset;
      }
  ...
  public boolean regionMatches(int thisStart, CharSequence string, int start, int length) {
    ...
    final var thatEnd = start + length;
  ...
  public boolean regionMatches(boolean ignoreCase, int thisStart, CharSequence string, int start, int length) {
    ...
    final var thisEnd = thisStart + length;
  ...
  public AsciiString replace(char oldChar, char newChar) {
    ...
    final var oldCharAsByte = c2b0(oldChar);
    final var newCharAsByte = c2b(newChar);
    final var len = offset + length;
  ...
  public AsciiString toLowerCase() {
    var lowercased = true;
    int i, j;
    final var len = length() + arrayOffset();
    ...
    final var newValue = new byte[length()];
  ...
  public int hashCode() {
    var h = hash;
    if (h == 0) {
      h = hashCodeAscii(value, offset, length);
      hash = h;
    }
    return h;
  }
  ...
  public String toString(int start, int end) {
    ...
    @SuppressWarnings("deprecation") final var str = new String(value, 0, start + offset, length);
    return str;
  }
  ...
  public char parseChar(int start) {
    ...
    final var startWithOffset = start + offset;
  ...
  public short parseShort(int start, int end, int radix) {
    var intValue = parseInt(start, end, radix);
  ...
  private int parseInt(int start, int end, int radix, boolean negative) {
    var max = Integer.MIN_VALUE / radix;
    var result = 0;
    var currOffset = start;
    while (currOffset < end) {
      var digit = Character.digit((char) (value[currOffset++ + offset] & 0xFF), radix);
      if (digit == -1) {
        throw new NumberFormatException(subSequence(start, end, false).toString());
      }
      if (max > result) {
  ...
  public static AsciiString cached(String string) {
    var asciiString = new AsciiString(string);
  ...
  private static AsciiString[] toAsciiStringArray(String[] jdkResult) {
    ...
    for (var i = 0; i < jdkResult.length; i++) {
  ...
  private static boolean regionMatchesCharSequences(final CharSequence cs, final int csStart,
    ...
    var csIndex = csStart;
    var csEnd = csIndex + length;
    var stringIndex = start;

    while (csIndex < csEnd) {
      var c1 = cs.charAt(csIndex++);
      var c2 = string.charAt(stringIndex++);
  ...
  public static int indexOfIgnoreCase(final CharSequence str, final CharSequence searchStr, int startPos) {
    ...
    final var endLimit = str.length() - searchStrLen + 1;
  ...
  public static int indexOf(final CharSequence cs, final char searchChar, int start) {
    ...
    for (var i = start < 0 ? 0 : start; i < sz; i++) {
