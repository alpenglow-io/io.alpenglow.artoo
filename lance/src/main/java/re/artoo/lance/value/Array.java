...
import re.artoo.lance.experimental.array.None;
import re.artoo.lance.experimental.array.Some;
...
public sealed interface Array<ELEMENT> extends Iterable<ELEMENT>, RandomAccess permits None, Some {
  ...
  default <TARGET> TARGET mutate(TARGET initial, TryConsumer2<? super TARGET, ? super ELEMENT> operation) {
    return switch (this) {
      case Some<ELEMENT>(var some) -> {
        operation.accept(initial, some[0]);
        yield tail().mutate(initial, operation);
      }
      default -> initial;
    };
  }
  ...
  default String join(String separator) {
    return switch (this) {
      case Some<ELEMENT>(var some) -> tail()
        .fold(new StringBuilder(some[0].toString()), (string, element) -> string.append("%s%s".formatted(separator, element)))
        .toString();
      default -> "";
    };
  }
