...
import artoo.type.Str;

import static org.assertj.core.api.Assertions.assertThat;
import static org.junit.jupiter.api.Assertions.assertThrows;
import static org.junit.jupiter.api.Assertions.fail;
import static artoo.func.Pred.not;
import static artoo.query.Many.from;
import static artoo.query.many.TestData.PACKAGES;
...
class AggregatableTest {
  ...
  void shouldReduceLongestName() {
    final var query = from("apple", "mango", "orange", "passionfruit", "grape")
      .aggregate(Str.Empty, String::toUpperCase, (longest, next) -> longest.length() > next.length() ? longest : next);
  ...
  void shouldReduceAverageWithSelector() {
    final var average = from("apple", "banana", "mango", "orange", "passionfruit", "grape")
      .average(String::length, Double::valueOf);
  ...
  void shouldBeNullSinceNoNumbers() {
    for (final var ignored : from("apple", "banana", "mango", "orange", "passionfruit", "grape").average())
      fail();
  }
  ...
  void shouldFindMaxAndMin() {
    ...
    for (final var value : max)
      assertThat(value).isEqualTo(14);
    for (final var value : min)
  ...
  void shouldFindMaxByNumber() {
    for (final var value : from(4294967296L, 466855135L, 81125L).max())
      assertThat(value).isEqualTo(4294967296L);
    for (final var value : from(4294967296L, 466855135L, 81125L).min())
      assertThat(value).isEqualTo(81125L);
  }

  @Test
  @DisplayName("should sum by selecting package weight")
  void shouldSumBySelectingWeight() {
    for (final var value : from(PACKAGES).sum(Package::weight))
