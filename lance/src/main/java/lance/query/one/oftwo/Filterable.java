...
public interface Filterable<A, B> extends Queryable.OfTwo<A, B> {
  default One.OfTwo<A, B> where(final Pred.TryBiPredicate<? super A, ? super B> where) {
    return where((index, first, second) -> where.tryTest(first, second));
  }

  default One.OfTwo<A, B> where(final Pred.TryTriPredicate<? super Integer, ? super A, ? super B> where) {
