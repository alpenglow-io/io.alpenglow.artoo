...
class PartitionableTest {
  ...
  void shouldTake3Rows() {
    final var query = query(59, 82, 70, 56, 92, 98, 85).take(3);
  ...
  void shouldTakeAllUntilOrange() {
    final var query = query("apple", "banana", "mango", "orange", "passionfruit", "grape").takeWhile(not("orange"::equals));
  ...
  void shouldTakeWithIndex() {
    ...
    final var query = query(fruits).takeWhile((index, fruit) -> fruit.length() >= index);
  ...
  void shouldSkipFirst3() {
    ...
    final var query = query(numbers).skip(3);
  ...
  void shouldSkipWhileNumbersLesserOrEqualThan90() {
    final var numbers = new Integer[] {59, 82, 70, 56, 92, 98, 85};
    final var query = query(numbers).skipWhile(number -> number <= 90);
  ...
  void shouldSkipWhileAmountGreaterThanIndexBy1000() {
    ...
    final var query = query(amounts).skipWhile((index, amount) -> amount > index * 1000);
