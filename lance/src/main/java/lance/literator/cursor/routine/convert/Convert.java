package lance.literator.cursor.routine.convert;
...
public sealed interface Convert<T, R> extends Routine<T, R> permits Convert.Listable, Convert.Arrayable {
  ...
  final class Listable<T> implements Convert<T, List<T>> {
    @Override
    public Func.TryFunction<T[], List<T>> onArray() {
      return ts -> List.of(Arrays.copyOf(ts, ts.length));
    }

    @Override
    public Func.TryFunction<Literator<T>, List<T>> onLiterator() {
      return ft -> {
    ...
    @Override
    public Func.TryFunction<Iterator<T>, List<T>> onIterator() {
      return it -> {
        final var list = new ArrayList<T>();
  ...
  final class Arrayable<T> implements Convert<T, T[]> {
    ...
    @Override
    public Func.TryFunction<T[], T[]> onArray() {
      return ts -> Arrays.copyOf(ts, ts.length);
    }

    @Override
    public Func.TryFunction<Literator<T>, T[]> onLiterator() {
      return li -> onIterator().apply(li);
    }

    @Override
    public Func.TryFunction<Iterator<T>, T[]> onIterator() {
      return it -> {
