...
import static oak.quill.query.Partitionable.with;
...
class PartitionableTest {
  ...
  void shouldTakeWithIndex() {
    ...
    final var query = from(fruits).takeWhile(with((index, fruit) -> fruit.length() >= index));
  ...
  void shouldSkipWhileAmountGreaterThanIndexBy1000() {
    ...
    final var query = from(amounts).skipWhile(with((index, amount) -> amount > index * 1000));
