...
public interface Matchable<A, B> extends Queryable.OfTwo<A, B> {
  default One.OfTwo<A, B> when(final Pred.TryBiPredicate<? super A, ? super B> pred, Cons.TryBiConsumer<? super A, ? super B> cons) {
    return () -> cursor().map(new WhenWhere<>(pair -> pred.tryTest(pair.first(), pair.second()), pair -> cons.tryAccept(pair.first(), pair.second())));
  }

  default <R> One.OfTwo<A, B> when(final Class<R> type, Cons.TryConsumer<? super R> cons) {
    return when(type, unary(cons));
  }

  default <P> One.OfTwo<A, B> when(final Class<P> type, Func.TryFunction<? super P, ? extends Pair<A, B>> func) {
