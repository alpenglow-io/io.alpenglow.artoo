...
public sealed interface Binary extends Random<Integer> permits Binary.Digit {
  @Override
  default <R> R let(final Func.TryFunction<? super Integer, ? extends R> func) {
    return let(32, func);
  }

  <R> R let(int bits, Func.TryFunction<? super Integer, ? extends R> func);
  ...
  final class Digit implements Binary {
    ...
    @Override
    public <R> R let(int bits, final Func.TryFunction<? super Integer, ? extends R> func) {
