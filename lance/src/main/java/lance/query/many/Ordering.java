...
public sealed interface Ordering<T> extends Many<T> {
  ...
  default <R> Ordering<T> by(Func.TryFunction<? super T, ? extends R> field) {
    return by(field, asc);
  }

  default <R> Ordering<T> by(Func.TryFunction<? super T, ? extends R> field, Arrange arrange) {
...
final class Order<T> implements Ordering<T> {
  ...
  @Override
  public <R> Ordering<T> by(final Func.TryFunction<? super T, ? extends R> field, final Arrange arrange) {
